...
import { audioFrame, audioStop, resume, setGain, suspend } from "@/audio";
import {
  HEIGHT,
  WIDTH,
  animationFrame,
...
import { isEmptyObj, isNotVoid, isVoid } from "@/utils/is";
import { downloadCanvas } from "@/utils/download";
import {
  P1_DEFAULT,
  P2_DEFAULT,
  useController,
} from "@/composable/use-controller";
import { fm2Parse, tasState } from "@/tas";
import { getNesData, loadNesData, nes, rom } from "@/nes";
import { useElement } from "@/composable/use-instance";
...
function screenshot(download?: boolean, imageName?: string) {
  ...
  if (download) {
    downloadCanvas(cvs.value, imageName);
  }
...
function fm2Stop() {
  nes.videoMode = false;
  nes.controllers[1].state = getFillArr(8, 0x40);
  nes.controllers[2].state = getFillArr(8, 0x40);
  addEvent();
}
