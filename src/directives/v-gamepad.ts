...
import { keyIn, objectKeys } from "@/utils/object";
...
function removeEventAll(el: HTMLElement, eventId: string) {
  if (_events[eventId]) {
    objectKeys(_events[eventId]).forEach((type) => {
 ...
 * ```
 */
export const vGamepad: Directive<
  HTMLElement,
...
> = (target, binding) => {
  ...
  if (binding.oldValue) {
    const oldKeys = resolveKeys(binding.oldValue).filter((key) =>
      keyIn(key, player)
    );
    const oldEventId = `gamepad-${`${
      arg + (checkPlayer ? "p2" : "p1")
    }-${oldKeys.join("-")}`}`;
    removeEventAll(target, oldEventId);
  }
  const keys = resolveKeys(binding.value).filter((key) => keyIn(key, player));
