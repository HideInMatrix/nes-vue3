...
import { onFrame } from "@/animation";
import { getSampleRate, onAudioSample } from "@/audio";
import {
  compressArray,
  compressNameTable,
  compressPtTile,
...
import { tasState } from "@/tas";
import type { ControllerStateType, EmitErrorObj, SaveData } from "@/types";
...
function loadNesData(
  saveData: SaveData,
  emitError: (error: EmitErrorObj) => void,
  url?: string
) {
  ...
  try {
    ...
    ppu.attrib = getFillArr(0x20, 0);
    ppu.bgbuffer = getFillArr(0xf000, 0);
    ppu.buffer = getFillArr(0xf000, 0);
    ppu.pixrendered = getFillArr(0xf000, 0);
...
export { nes, nesFrame, getNesData, loadNesData, rom, controllerState };
