...
import { useInstance } from "@/composable/use-instance";
import { vGamepad } from "@/directives/v-gamepad";
import type { Controller, EmitErrorObj } from "@/types";
...
function playVideo() {
  nes.value.fm2URL("happylee-supermariobros,warped.fm2", 0).then((fm2Play) => {
    fm2Play();
  });
}
...
<template>
  ...
  <div>
    <button v-gamepad="btnA">A</button>
    <button v-gamepad="btnB">B</button>
    <button v-gamepad="['A', 'B']">AB</button>
    <button v-gamepad="['C', 'D']">CD</button>
    <button @click="testVGamepad">Change A To C</button>
    <button @click="cheatCode">Cheat Code</button>
    <button @click="disableCheatCode">Disable Cheat Code</button>
  </div>
</template>
