...
import type { NesVueProps, Player } from "@/types";
import { controllerState } from "@/nes";
import { fillFalse, gpFilter } from "@/utils";
...
class GamepadManager {
  ...
  gamepadBtns: ComputedRef<{ p1: string[]; p2: string[] }>;

  constructor(gamepadBtns: ComputedRef<{ p1: string[]; p2: string[] }>) {
    window.addEventListener(
      "gamepadconnected",
      this.connectHandler.bind(this, true)
    );
    window.addEventListener(
      "gamepaddisconnected",
      this.connectHandler.bind(this, false)
    );
  ...
  axesHandler(player: Player, check: boolean, aindex: number, bindex: number) {
    ...
    if (check) {
      if (!hold) {
        emitControllerState(this.gamepadBtns.value[player][bindex], 0x41);
        this.axesHolding[player][aindex] = true;
      }
    } else if (hold) {
      emitControllerState(this.gamepadBtns.value[player][bindex], 0x40);
...
export function useController(
  props: NesVueProps
): (eventCode: string, state: 0x40 | 0x41) => void {
  ...
  function setTurboInterval() {
    interval = 1000 / (2 * mathBetween(props.turbo as number, 5, 20));
  }

  function setController() {
    controllerState.init();
    objectKeys(KEYS_INDEX).forEach((key) => {
  ...
  const gamepad = new GamepadManager(gamepadBtns);
